...
export const ConnectionsComponent = createComponent(() => {
  ...
  const isDisabled$ = testLoading$.pipe(
    mergeMap((loading) => apiKeys$.pipe(map((apiKeys) => loading.openai || loading.gemini || (!apiKeys.openai && !apiKeys.gemini))))
  );

  const buttonText$ = testLoading$.pipe(map((loading) => (loading.openai || loading.gemini ? "Testing..." : "Test Connections")));
